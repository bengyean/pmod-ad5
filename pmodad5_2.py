...
def set_channel(channel):
    ...
    manipulatedData = read_register_value(regAddress, \
        registerSize[regAddress], 1)
    manipulatedData &= 0xFC00FF   # Keep all bit values except Channel bits
    manipulatedData |= channelBits

    set_register_value(regAddress, manipulatedData,  \
        registerSize[regAddress], 1)
    time.sleep(0.01)
...
def initiate_single_conversion():
    ...
    print('   Initiate Single Conversion... \
        (Device will go into low power mode when conversion complet)')

    manipulateData = 0
    regAddress = AD7193_REG_MODE

    manipulateData = read_register_value(regAddress, \
        registerSize[regAddress], 1)
    manipulateData &= 0x1FFFFF   # Keep all bit values except Mode bits
    manipulateData |= 0x200000   # single conversion mode bits

    set_register_value(regAddress, manipulateData, \
        registerSize[regAddress], 1)

    manipulateData =  read_register_value(regAddress, manipulateData, \
        registerSize[regAddress], 1)
    ...
    rdyState = read_register_value(regAddress, \
        registerSize[regAddress], 1)
